...
mo_ta_ten_nghi_khac varchar(45),
...
(4,"Villa No Beach",22000,9000000,8,"normal","Có hồ bơi",300,3,3,1),
...
where SUBSTRING_INDEX(nhan_vien.ho_ten, ' ', - 1) regexp '^[HTK]' and LENGTH(nhan_vien.ho_ten) <= 15;

-- lấy tên của họ tên
select SUBSTRING_INDEX(ho_ten, ' ', - 1) from nhan_vien;
...
select khach_hang.ma_khach_hang,khach_hang.ho_ten, count(loai_khach.ma_loai_khach) as so_lan 
from khach_hang
...
select kh.ma_khach_hang, kh.ho_ten, hd.ma_hop_dong, dv.ten_dich_vu,hd.ngay_lam_hop_dong,hd.ngay_ket_thuc,
...
from hop_dong hd inner join dich_vu dv  on hd.ma_dich_vu=dv.ma_dich_vu inner join 
...
select dv.ma_dich_vu, dv.ten_dich_vu,  dv.dien_tich,dv.so_nguoi_toi_da , ldv.ten_loai_dich_vu
from hop_dong hd inner join dich_vu dv  on hd.ma_dich_vu=dv.ma_dich_vu inner join 
loai_dich_vu ldv on dv.ma_loai_dich_vu=ldv.ma_loai_dich_vu
...
select month(hop_dong.ngay_lam_hop_dong) as 'tháng', count(hop_dong.ma_khach_hang) as 'số lượng khách hàng'
from hop_dong
...
select dvdk.ma_dich_vu_di_kem, dvdk.ten_dich_vu_di_kem from dich_vu_di_kem dvdk inner join 
hop_dong_chi_tiet hdct on dvdk.ma_dich_vu_di_kem=hdct.ma_dich_vu_di_kem inner join
hop_dong on hdct.ma_hop_dong=hop_dong.ma_hop_dong inner join
khach_hang on hop_dong.ma_khach_hang=khach_hang.ma_khach_hang inner join
